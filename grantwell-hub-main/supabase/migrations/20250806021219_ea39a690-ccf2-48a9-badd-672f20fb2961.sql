...
-- Create a trigger to auto-setup monitoring for new users
...
-- Create trigger for new approved users (simplified)
...
-- Also create a trigger for status updates
...
DROP TRIGGER IF EXISTS trigger_auto_setup_monitoring_update ON profiles;
CREATE TRIGGER trigger_auto_setup_monitoring_update
  AFTER UPDATE ON profiles
  FOR EACH ROW
  EXECUTE FUNCTION auto_setup_monitoring_on_approval();