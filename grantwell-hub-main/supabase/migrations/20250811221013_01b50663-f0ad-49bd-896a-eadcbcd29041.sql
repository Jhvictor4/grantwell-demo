-- Create DOJ file mappings table and Subrecipient tables with tenant-scoped RLS
...
-- RLS: org-scoped CRUD following existing pattern (matches grant_matches policy)
CREATE POLICY IF NOT EXISTS org_crud_file_mappings ON public.file_mappings
  FOR ALL
  USING (
    EXISTS (
...
CREATE POLICY IF NOT EXISTS org_crud_subrecipients ON public.subrecipients
  FOR ALL
  USING (
    EXISTS (
      SELECT 1 FROM public.grants g
...
CREATE POLICY IF NOT EXISTS org_crud_subrecipient_monitoring ON public.subrecipient_monitoring
  FOR ALL
  USING (
    EXISTS (
      SELECT 1 FROM public.grants g
