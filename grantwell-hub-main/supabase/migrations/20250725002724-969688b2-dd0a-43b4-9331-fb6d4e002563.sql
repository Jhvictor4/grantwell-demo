...
CREATE TABLE public.milestones (
  ...
  milestone_type TEXT NOT NULL DEFAULT 'deliverable' CHECK (milestone_type IN ('deliverable', 'outcome', 'reporting', 'compliance')),
...
CREATE POLICY "Users can view expenses for accessible grants"
...
CREATE POLICY "Users can view milestones for accessible grants"
  ON public.milestones FOR SELECT
  USING (
    EXISTS (
      SELECT 1 FROM public.grant_team_assignments gta 
      WHERE gta.grant_id = milestones.grant_id 
      AND gta.user_id = auth.uid()
    ) OR 
    get_user_role(auth.uid()) = ANY (ARRAY['admin'::app_role, 'manager'::app_role])
  );
