-- Add sector column to discovered_grants table (only if it doesn't exist)
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'discovered_grants' AND column_name = 'sector') THEN
...
WHERE 
  sector IS NULL OR sector = 'Other' AND (
...
-- Create or replace function to auto-categorize new grants
...
-- Create trigger to auto-categorize grants on insert/update (drop first if exists)
...
-- Create justgrants_status table for sync status tracking (only if it doesn't exist)
...
DROP POLICY IF EXISTS "Admin and managers can manage justgrants status" ON public.justgrants_status;
DROP POLICY IF EXISTS "All authenticated users can view justgrants status" ON public.justgrants_status;
...
DROP TRIGGER IF EXISTS update_justgrants_status_updated_at ON public.justgrants_status;
