...
BEGIN
  ...
  IF NEW.application_stage IN ('preparation', 'in_progress', 'submission', 'awarded', 'rejected') 
    ...
    INSERT INTO grants (
      ...
      discovered_grant_record.deadline,
    ...
    -- Narrative tasks
    (new_grant_id, 'Project Narrative Draft', 'Create the main project narrative document', 'pending', 'high', NEW.user_id, NOW(), NOW()),
    (new_grant_id, 'Quarterly Report Planning', 'Plan quarterly reporting structure and timeline', 'pending', 'medium', NEW.user_id, NOW(), NOW()),
    
    -- Budget tasks  
    (new_grant_id, 'Budget Planning', 'Create detailed budget breakdown', 'pending', 'high', NEW.user_id, NOW(), NOW()),
    (new_grant_id, 'Vendor Proposals', 'Collect vendor quotes and proposals', 'pending', 'medium', NEW.user_id, NOW(), NOW()),
    
    -- Document tasks
    (new_grant_id, 'Document Collection', 'Gather required supporting documents', 'pending', 'medium', NEW.user_id, NOW(), NOW()),
    (new_grant_id, 'File Upload Preparation', 'Prepare final document uploads', 'pending', 'low', NEW.user_id, NOW(), NOW()),
    
    -- Administrative tasks
    (new_grant_id, 'Final Review', 'Complete final review and compliance check', 'pending', 'high', NEW.user_id, NOW(), NOW()),
    (new_grant_id, 'Authorization Approval', 'Obtain necessary approvals for submission', 'pending', 'high', NEW.user_id, NOW(), NOW());
...
BEGIN
  ...
  LOOP
    ...
    (new_grant_id, 'Project Narrative Draft', 'Create the main project narrative document', 'pending', 'high', bookmark_record.user_id, NOW(), NOW()),
    (new_grant_id, 'Budget Planning', 'Create detailed budget breakdown', 'pending', 'high', bookmark_record.user_id, NOW(), NOW()),
    (new_grant_id, 'Document Collection', 'Gather required supporting documents', 'pending', 'medium', bookmark_record.user_id, NOW(), NOW()),
    (new_grant_id, 'Final Review', 'Complete final review and compliance check', 'pending', 'high', bookmark_record.user_id, NOW(), NOW());
