-- Finalize pending DB items: create file_mappings and fix subrecipient monitoring policies (idempotent)
-- Safe to run multiple times

-- 1) Ensure gen_random_uuid is available
CREATE EXTENSION IF NOT EXISTS pgcrypto;

-- 2) Create file_mappings table if missing
...
-- Drop policies if they already exist to recreate cleanly
DO $$
BEGIN
  IF EXISTS (
...
-- Recreate policies mirroring org access used elsewhere (e.g., grant_award_setup, grant_matches)
...
-- 3) Fix subrecipient monitoring (only if table exists)
DO $$
BEGIN
  IF to_regclass('public.subrecipient_monitoring') IS NOT NULL THEN
    -- Ensure grant_id exists for RLS reference
    IF NOT EXISTS (
      SELECT 1
      FROM information_schema.columns
      WHERE table_schema = 'public' AND table_name = 'subrecipient_monitoring' AND column_name = 'grant_id'
    ) THEN
      ALTER TABLE public.subrecipient_monitoring ADD COLUMN grant_id uuid;
    END IF;
    ...
    EXECUTE $$
      CREATE POLICY "org_crud_subrecipient_monitoring"
      ON public.subrecipient_monitoring
      FOR ALL
      USING (
        EXISTS (
          SELECT 1 FROM public.grants g
    ...
    IF NOT EXISTS (
      ...
      CREATE INDEX idx_subrecipient_monitoring_grant_id ON public.subrecipient_monitoring(grant_id);
    END IF;
  END IF;
END$$;

-- 4) If subrecipients table exists, ensure policy aligns (no-op otherwise)
DO $$
BEGIN
  IF to_regclass('public.subrecipients') IS NOT NULL THEN
    ...
    EXECUTE $$
      CREATE POLICY "org_crud_subrecipients"
      ON public.subrecipients
      FOR ALL
      USING (
        EXISTS (
          SELECT 1 FROM public.grants g
    ...
    IF NOT EXISTS (
      ...
      CREATE INDEX idx_subrecipients_grant_id ON public.subrecipients(grant_id);
    END IF;
  END IF;
END$$;