...
DROP POLICY IF EXISTS "Users can view notes for accessible grants" ON public.grant_notes;
CREATE POLICY "Users can view notes for accessible grants"
...
DROP POLICY IF EXISTS "Users can create notes for accessible grants" ON public.grant_notes;
CREATE POLICY "Users can create notes for accessible grants"
ON public.grant_notes
FOR INSERT
WITH CHECK (
  created_by = auth.uid()
...
DROP POLICY IF EXISTS "Users can update notes for accessible grants" ON public.grant_notes;
CREATE POLICY "Users can update notes for accessible grants"
ON public.grant_notes
FOR UPDATE
USING (
  user_has_grant_permission(auth.uid(), grant_id, 'edit')
...
DROP TRIGGER IF EXISTS update_grant_notes_updated_at ON public.grant_notes;
